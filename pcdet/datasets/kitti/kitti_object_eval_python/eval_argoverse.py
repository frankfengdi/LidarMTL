...
def clean_data(gt_anno, dt_anno, current_class, difficulty):
    ...
    for i in range(num_gt):
        ...
        dist = np.sqrt(location[0]**2 + location[1]**2 + location[2]**2)
        
        if (dist < DISTANCE_RANGE[difficulty]) or (dist > DISTANCE_RANGE[difficulty+1]):
            ignore = True
        if valid_class == 1 and not ignore:
            ...
            ignored_gt.append(-1)
        if gt_anno["name"][i] == "DontCare": #OTHER_MOVER,ON_ROAD_OBSTACLE
...
def get_distance_eval_result(gt_annos, dt_annos, current_classes, PR_detail_dict=None):
    # evaluation according to detection distance
    # easy: 0-30m
    # moderate: 30-50m
    # hard: 50-70m
    overlap_0_7 = np.array([[0.7, 0.5, 0.5], [0.7, 0.5, 0.5],[0.7, 0.5, 0.5]]) #[2d, bev, 3d] * num_class
    overlap_0_5 = np.array([[0.5, 0.5, 0.5], [0.5, 0.25, 0.25],[0.5, 0.25, 0.25]])
